...
public:
	...
	void append_item(uint8_t*, uint64_t);	
	void append_item(const char*);	
	void append_item(string);
	void prepend_item(uint8_t*, uint64_t);
	...
	void extract_item(vector<uint64_t>&, uint64_t);
	void extract_item(uint8_t*, uint64_t);	
	void extract_item(char*, uint64_t);	
	...
	void truncate();
	void clear_pkt();
	struct ip* allocate_ip_hdr_mem(uint64_t);
	~Packet();
};
